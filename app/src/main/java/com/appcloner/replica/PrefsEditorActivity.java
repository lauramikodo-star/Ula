...
public class PrefsEditorActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        authority = resolveProviderAuthority(targetPackage);
        Log.d(TAG, "Resolved provider authority for " + targetPackage + ": " + authority);
    ...
    private String resolveProviderAuthority(String pkg) {
        final String suffix = "." + PROVIDER_AUTHORITY;
        if (pkg == null) {
        ...
        String combined = pkg + "." + PROVIDER_AUTHORITY;
        try {
            ProviderInfo directInfo = pm.resolveContentProvider(combined, 0);
            if (directInfo != null && directInfo.authority != null) {
                return directInfo.authority;
            }
        } catch (Throwable t) {
            Log.w(TAG, "Direct resolve of provider authority failed for " + combined, t);
        }

        // Fallback: try un-prefixed default authority (for older or already-injected providers)
        try {
            ProviderInfo defaultInfo = pm.resolveContentProvider(PROVIDER_AUTHORITY, 0);
            if (defaultInfo != null && defaultInfo.authority != null) {
                return defaultInfo.authority;
            }
        } catch (Throwable t) {
            Log.w(TAG, "Fallback resolve of default provider authority failed", t);
        }

        try {
            PackageInfo packageInfo;
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                packageInfo = pm.getPackageInfo(pkg, PackageManager.PackageInfoFlags.of(PackageManager.GET_PROVIDERS));
            } else {
                //noinspection deprecation
                packageInfo = pm.getPackageInfo(pkg, PackageManager.GET_PROVIDERS);
            }
            if (packageInfo != null && packageInfo.providers != null) {
                for (ProviderInfo provider : packageInfo.providers) {
                    if (provider == null || provider.authority == null) continue;
                    if (PROVIDER_AUTHORITY.equals(provider.name)
                            || PROVIDER_AUTHORITY.equals(provider.authority)
                            || provider.authority.endsWith(suffix)) {
                        return provider.authority;
                    }
                }
            }
        } catch (PackageManager.NameNotFoundException e) {
            Log.w(TAG, "Package not found while resolving provider authority: " + pkg, e);
        } catch (Throwable t) {
            Log.w(TAG, "Failed to enumerate providers for " + pkg, t);
        }

        // Final fallback to combined authority to keep behavior consistent
        return combined;
    }
    ...
    private void loadFiles() {
        new Thread(() -> {
            try {
                // First, try to ping the provider to verify connectivity
                Bundle pingResult = null;
                try {
                    pingResult = getContentResolver().call(providerUri(), "ping", null, null);
                } catch (Throwable ignored) {}
                
                if (pingResult == null || !pingResult.getBoolean("ok", false)) {
                    final Bundle finalPingResult = pingResult;
                    runOnUiThread(() -> {
                        String providerError = finalPingResult != null ? finalPingResult.getString("error") : null;
                        String errorMsg = "Cannot connect to cloned app provider.\n\n" +
                                "Possible causes:\n" +
                                "1. The app was not properly cloned\n" +
                                "2. The cloned app is not running\n" +
                                "3. IPC permission issue\n\n" +
                                "Try restarting the cloned app.";
                        if (providerError != null && !providerError.isEmpty()) {
                ...
                Log.e(TAG, "Security exception - permission denied", se);
                runOnUiThread(() -> {
                    new AlertDialog.Builder(this)
                            .setTitle("Permission Denied")
                            .setMessage("Cannot access the cloned app's data.\n\n" +
                                    "The IPC permission may not be properly configured.")
                            .setPositiveButton("OK", (d, w) -> finish())
                            .show();
                });
            } catch (IllegalArgumentException iae) {
                Log.e(TAG, "Provider not found", iae);
                runOnUiThread(() -> {
                    new AlertDialog.Builder(this)
                            .setTitle("Provider Not Found")
                            .setMessage("The cloned app's provider was not found.\n\n" +
                                    "Make sure the app was properly cloned.")
                            .setPositiveButton("OK", (d, w) -> finish())
                            .show();
                });
            } catch (Throwable t) {
                Log.e(TAG, "loadFiles error", t);
                runOnUiThread(() -> Toast.makeText(this, "Failed to query provider: " + t.getMessage(), Toast.LENGTH_LONG).show());
            }
        }).start();
    }
